    ...
    pub(crate) fn stresstest_cmd(&self, args: &[&str]) {
        ...
        match args[0] {
            ...
            "compare" => {
                ...
                let was_enabled = crate::autonomy::AUTONOMOUS_CONTROL.is_enabled(); let which = args[1]; unsafe { crate::uart_print(b"\n[COMPARE] Running with autonomy DISABLED...\n"); } crate::autonomy::AUTONOMOUS_CONTROL.disable();
                let metrics_off = match which { "memory" => { let mut cfg = crate::stress_test::StressTestConfig::new(crate::stress_test::StressTestType::Memory); cfg.duration_ms = duration_ms; cfg.target_pressure = target_pressure; crate::stress_test::run_memory_stress(cfg) }, "commands" => { let mut cfg = crate::stress_test::StressTestConfig::new(crate::stress_test::StressTestType::Commands); cfg.duration_ms = duration_ms; cfg.command_rate = rate; crate::stress_test::run_command_stress(cfg) }, "multi" => { let mut cfg = crate::stress_test::StressTestConfig::new(crate::stress_test::StressTestType::MultiSubsystem); cfg.duration_ms = duration_ms; crate::stress_test::run_multi_stress(cfg) }, _ => { unsafe { crate::uart_print(b"[ERROR] Unknown test type\n"); } return; } };
                ...
                let metrics_on = match which { "memory" => { let mut cfg = crate::stress_test::StressTestConfig::new(crate::stress_test::StressTestType::Memory); cfg.duration_ms = duration_ms; cfg.target_pressure = target_pressure; crate::stress_test::run_memory_stress(cfg) }, "commands" => { let mut cfg = crate::stress_test::StressTestConfig::new(crate::stress_test::StressTestType::Commands); cfg.duration_ms = duration_ms; cfg.command_rate = rate; crate::stress_test::run_command_stress(cfg) }, "multi" => { let mut cfg = crate::stress_test::StressTestConfig::new(crate::stress_test::StressTestType::MultiSubsystem); cfg.duration_ms = duration_ms; crate::stress_test::run_multi_stress(cfg) }, _ => unreachable!(), };
                if !was_enabled { crate::autonomy::AUTONOMOUS_CONTROL.disable(); }
                unsafe { crate::uart_print(b"\n=== Comparative Results ===\n"); match which { "memory" => { crate::uart_print(b"  Peak pressure: off="); self.print_number_simple(metrics_off.peak_memory_pressure as u64); crate::uart_print(b"% on="); self.print_number_simple(metrics_on.peak_memory_pressure as u64); crate::uart_print(b"%\n"); crate::uart_print(b"  OOM events: off="); self.print_number_simple(metrics_off.oom_events as u64); crate::uart_print(b" on="); self.print_number_simple(metrics_on.oom_events as u64); crate::uart_print(b"\n"); }, "commands" | "multi" => { crate::uart_print(b"  Actions: off="); self.print_number_simple(metrics_off.actions_taken as u64); crate::uart_print(b" on="); self.print_number_simple(metrics_on.actions_taken as u64); crate::uart_print(b"\n"); }, _ => {} } crate::uart_print(b"  Duration_ms: off="); self.print_number_simple(metrics_off.test_duration_ms); crate::uart_print(b" on="); self.print_number_simple(metrics_on.test_duration_ms); crate::uart_print(b"\n\n"); }
            }
            "report" => {
