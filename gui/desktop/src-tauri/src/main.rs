...
// Prefer IPv4 loopback to avoid ::1 localhost differences on macOS
const DAEMON_URL: &str = "http://127.0.0.1:8871";
...
async fn launch_daemon() -> Result<String, String> {
    ...
    #[cfg(debug_assertions)]
    {
        let child = StdCommand::new("cargo")
            .args(["run", "--manifest-path", "apps/daemon/Cargo.toml", "--bin", "sisctl"])
            .spawn()
