...
import { Plus, Play, TrendingUp, MessageSquare, Download, Copy } from 'lucide-react';

interface GraphPanelProps {
  onGraphStateEvent?: (event: any) => void;
}
...
export function GraphPanel({ onGraphStateEvent }: GraphPanelProps) {
  ...
  const predict = useMutation({
    ...
    onSuccess: (data) => {
      setPredictionResult(`Predicted: ${data.predicted?.toFixed(2) ?? 'N/A'}${data.conf ? ` (conf: ${data.conf.toFixed(2)})` : ''}`);
      setError(null);
    },
    onError: (err: any) => {
  ...
  const operators = graphState?.operators || [];
  ...
  const channels = graphState?.channels || [];
                          ...
                          <div className="flex gap-4 text-sm">
                            ...
                            {(op as any).stats && (
                              <span className="text-green-500">
                                {(op as any).stats.execCount} runs, {(op as any).stats.avgUs.toFixed(1)}μs avg
                              </span>
                            )}
                          </div>
                        </div>
