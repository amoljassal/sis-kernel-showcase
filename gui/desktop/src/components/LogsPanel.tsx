...
import { logsApi, type LogEntry, type StartRunRequest } from '../lib/api';
...
export function LogsPanel() {
  ...
  useWebSocket((event) => {
    ...
    if ((event as any).type === 'self_check_completed') {
      setSelfCheckPassed((event as any).success);
    }

    // Backpressure detection
    if ((event as any).type === 'backpressure') {
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <span className="font-mono text-sm font-semibold w-24 truncate" title={run?.runId || "unknown"}>
                          {run?.runId || "unknown".slice(0, 12)}...
                        </span>
                      ...
                      <button
                        onClick={() => exportRun.mutate(run?.runId || "unknown")}
